class Play extends Phaser.Scene {
  ...
  create() {
    ...
    this.emitter.on("switch-state", this.switchState.bind(this));
  }
  ...
  saveGame(slotName = "default") {
    const gameState = {
      player: this.player.serialize(), // Player position and state
      grid: this.grid.map((row) => row.map((cell) => cell.serialize())), // Serialize the entire grid
      turnsTaken: this.scene.get("uiScene").turnsTaken,
      seeds: this.scene.get("uiScene").seeds,
    };
    this.emitter.emit("save-game", slotName, gameState);
    console.log(`Game saved to slot: ${slotName}`);
  }

  loadGame(slotName = "default") {
    this.emitter.emit("load-game", slotName);
  }

  deleteGame(slotName = "default") {
    this.emitter.emit("delete-save", slotName);
  }
  ...
  switchState(state) {
    ...
    for (let i = 0; i < this.grid.length; i++) {
      for (let j = 0; j < this.grid[i].length; j++) {
        this.grid[i][j].deserialize(state.grid[i][j]);
      }
    }

    // Restore UI state
    const uiScene = this.scene.get("uiScene");
    uiScene.seeds = state.seeds;
    uiScene.turnsTaken = state.turnsTaken;
    uiScene.seedText.text = `Seeds: ${uiScene.seeds}`;
    uiScene.turnsText.text = `Turns: ${uiScene.turnsTaken}`;
  }
