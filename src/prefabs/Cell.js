class Cell extends Phaser.GameObjects.Sprite {
  constructor(scene, x, y, texture) {
    ...
    this.sunText = scene.add.text(x - 60, y - 60, this.sun.toString(), {
      fontSize: "18px",
      color: "yellow",
    });
    this.waterText = scene.add.text(x - 40, y - 60, this.water.toString(), {
      fontSize: "18px",
      color: "blue",
    });
  }
  ...
  NextTurn() {
    this.ChangeSun();
    this.ChangeWater();
    if (this.plant) this.plant.GiveNutrients(this, this.sun, this.water);
    //this.updateText()
  }
  ...
  deserialize(data) {
    ...
    if (data.plant) {
      if (!this.plant) {
        this.plant = new Plant(this.scene, this.x, this.y); // Or however plants are created
      }
      this.plant.deserialize(data.plant);
    } else {
      this.plant = null;
    }
    this.updateText();
